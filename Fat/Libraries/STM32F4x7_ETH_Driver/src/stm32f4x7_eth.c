...
typedef struct
{
  ...
  ETH_DMA_Rx_Frame_infos        RxFrameInfos;  /*!< last Rx frame infos         */
...
/**
  * @brief  Clears the ETHERNETӳ DMA pending flag.
...
/**
  * @brief  Clears the ETHERNETӳ DMA IT pending bit.
...
void ETH_ConfigMAC(uint32_t   Speed,uint32_t DuplexMode)
{
		uint32_t tmpreg = 0;
		/*------------------------ ETHERNET MACCR Configuration --------------------*/
		/* Get the ETHERNET MACCR value */
		...
		tmpreg |= (uint32_t)(Speed | DuplexMode);

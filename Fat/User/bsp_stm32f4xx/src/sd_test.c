...
#define NUMBER_OF_BLOCKS      20  /* For Multi Blocks operation (Read/Write) */
...
void sd_test(void)
{
	...
	{
		printf("sd card init failed ! please insert the card or change the card");
	}
	SD_EraseTest();
...
{  
  ...
  if (Status == SD_OK)
  {
    ...
    while(SD_GetStatus() != SD_TRANSFER_OK);
		...
		printf("start : %d  , end: %d \n",start,end);
		speed = (BLOCK_SIZE*NUMBER_OF_BLOCKS)/(end - start)*1000/1024/1024;

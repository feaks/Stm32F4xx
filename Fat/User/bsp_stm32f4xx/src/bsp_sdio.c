...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  DMA_ClearFlag(SD_SDIO_DMA_STREAM, SD_SDIO_DMA_FLAG_FEIF | SD_SDIO_DMA_FLAG_DMEIF | SD_SDIO_DMA_FLAG_TEIF | SD_SDIO_DMA_FLAG_HTIF | SD_SDIO_DMA_FLAG_TCIF);
	DMA_ClearITPendingBit(SD_SDIO_DMA_STREAM,SD_SDIO_DMA_IT_FEIF| SD_SDIO_DMA_IT_DMEIF | SD_SDIO_DMA_IT_TEIF |SD_SDIO_DMA_IT_HTIF | SD_SDIO_DMA_IT_TCIF);
...
SD_Error SD_PowerON(void)
{
  ...
  /*!< If errorst    atus is SD_OK it is a SD card: SD card 2.0 (voltage range mismatch)
     or SD card 1.x */
  if (errorstatus == SD_OK)
  {
    /*!< SD CARD */
...
void SD_ProcessDMAIRQ(void)
{
	...
	if(DMA_GetITStatus(SD_SDIO_DMA_STREAM,SD_SDIO_DMA_IT_TCIF) != RESET )
	{
		DMAEndOfTransfer = 0x01;
		
	}
	/* Transfer error interrupt */
	if(DMA_GetITStatus(SD_SDIO_DMA_STREAM,SD_SDIO_DMA_IT_TEIF) != RESET )
	{

	}
	/* FIFO error interrupt */
	if(DMA_GetITStatus(SD_SDIO_DMA_STREAM,SD_SDIO_DMA_IT_FEIF) != RESET )
	{
		
	}
	DMA_ClearITPendingBit(SD_SDIO_DMA_STREAM,SD_SDIO_DMA_IT_FEIF | SD_SDIO_DMA_IT_TEIF  | SD_SDIO_DMA_IT_TCIF);
}
...
//  timeout = SDIO_CMD0TIMEOUT*168; /*!< 10000 */

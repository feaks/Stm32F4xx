...
TickType_t 		pxPreviousWakeTime = xTaskGetTickCount();
	...
	/* Initialise the UART. */
	CLI_Init(configCLI_BAUD_RATE);

	/* Send the welcome message. */
	UART1_DMA_Send(( uint8_t * ) pcWelcomeMessage, strlen( pcWelcomeMessage ) );
	for( ;; )
	{
		...
		while(index != 30 - DMA2_Stream5->NDTR)
		{
//			/* Ensure exclusive access to the UART Tx. */
//			if( xSemaphoreTake( xTxMutex, cmdMAX_MUTEX_WAIT ) == pdPASS )
//			{
				cRxedChar = rx_buf[index];
				if(++index  == 30)
					index = 0;
...
//					UART1_DMA_Send( ( uint8_t * ) pcNewLine, strlen( pcNewLine ) );
					...
					do
					{
						...
						xReturned = FreeRTOS_CLIProcessCommand( cInputString, pcOutputString, 400 );

						/* Write the generated string to the UART. */
						UART1_DMA_Send(( uint8_t * ) pcOutputString, strlen( pcOutputString ) );
					...
					UART1_DMA_Send(  ( uint8_t * ) pcEndOfOutputMessage, strlen( pcEndOfOutputMessage ) );
				}
				else
				{
					if( cRxedChar == '\r' )
